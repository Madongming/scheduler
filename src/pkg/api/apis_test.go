...
func TestRunJob(t *testing.T) {
	...
	// 创建两个任务
	if err := CreateJob("job1", "任务1", plugin.SleepCmd, 0, 5*time.Second, time.Second, 3); err != nil {
		t.Errorf("CreateJob(job1, 任务1, plugin.SleepCmd, 0, 5*time.Second, time.Second, 3) error = %v", err)
	}
	if err := CreateJob("job2", "任务2", plugin.SleepCmd, 0, 5*time.Second, time.Second, 3); err != nil {
		t.Errorf("CreateJob(job2, 任务2, plugin.SleepCmd, 0, 5*time.Second, time.Second, 3) error = %v", err)
	}
